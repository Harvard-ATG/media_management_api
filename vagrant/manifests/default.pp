...
exec {'drop-project-db':
    ...
    command => 'psql -d postgres -c "DROP DATABASE IF EXISTS media_management_api"',
    user => 'postgres',
    group => 'postgres',
    logoutput => true,
}
...
exec {'drop-existing-project-user':
    ...
    command => 'psql -d postgres -c "DROP USER IF EXISTS media_management_api"',
    user => 'postgres',
    group => 'postgres',
    logoutput => true,
}
...
exec {'create-project-user':
    ...
    command => 'psql -d postgres -c "CREATE USER media_management_api WITH PASSWORD \'media_management_api\'"',
    user => 'postgres',
    group => 'postgres',
    logoutput => true,
}
...
exec {'create-project-db':
    ...
    command => 'psql -d postgres -c "CREATE DATABASE media_management_api WITH OWNER media_management_api"',
    user => 'postgres',
    group => 'postgres',
    logoutput => true,
}
...
# Create a symlink from ~/media_management_api to /vagrant as a convenience for the developer
file {'/home/vagrant/media_management_api':
    ensure => link,
    target => '/vagrant',
}

# Create a virtualenv for media_management_api
exec {'create-virtualenv':
    provider => 'shell',
    user => 'vagrant',
    group => 'vagrant',
    require => [ Package['virtualenvwrapper'], File['/home/vagrant/media_management_api'], Exec['known_hosts'], Exec['create-project-db']],
    environment => ["HOME=/home/vagrant","WORKON_HOME=/home/vagrant/.virtualenvs"],
    command => 'bash /vagrant/vagrant/venv_bootstrap.sh',
    creates => '/home/vagrant/.virtualenvs/media_management_api',
}

# set the DJANGO_SETTINGS_MODULE environment variable
file_line {'add DJANGO_SETTINGS_MODULE env to postactivate':
    ensure => present,
    line => 'export DJANGO_SETTINGS_MODULE=media_management_api.settings.local',
    path => '/home/vagrant/.virtualenvs/media_management_api/bin/postactivate',
...
file_line {'add DJANGO_SETTINGS_MODULE env to postdeactivate':
    ...
    path => '/home/vagrant/.virtualenvs/media_management_api/bin/postdeactivate',
    require => Exec['create-virtualenv'],
}
...
file {'/home/vagrant/.bash_profile':
    owner => 'vagrant',
    content => '
echo "Activating python virtual environment \"media_management_api\""
