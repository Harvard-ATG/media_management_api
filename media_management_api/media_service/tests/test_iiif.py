...
from media_management_api.media_service.models import Collection
...
class IiifManifestViewTest(TestCase):
    ...
    def test_manifest(self):
        ...
        request = self.factory.get(
            reverse("api:iiif:manifest", kwargs={"manifest_id": collection.pk})
        )
        ...
        self.assertEqual(
            response.data["@context"], "http://iiif.io/api/presentation/2/context.json"
        )
...
class IIIFManifestTest(unittest.TestCase):
        ...
        images = [
            {
                "id": 1,
                "is_iiif": True,
                "url": "http://localhost:8000/loris/foo.jpg",
                "label": "foo.jpg",
                "description": "",
                "metadata": [],
            },
            {
                "id": 2,
                "is_iiif": True,
                "url": "http://localhost:8000/loris/bar.jpg",
                "label": "bar.jpg",
                "description": "",
                "metadata": [],
            },
    ...
    def test_create_manifest_with_images(self):
        ...
        manifest_id = 1
        request = self.factory.get(
            reverse("api:iiif:manifest", kwargs={"manifest_id": manifest_id})
        )
        manifest = self.create_manifest(
            request, manifest_id, images=self.get_images_list()
        )
        md = manifest.to_dict()
        ...
        self.assertEqual(
            len(md["sequences"][0]["canvases"]),
            len(iiif_images),
            "one canvas per image",
        )
        for idx, canvas in enumerate(md["sequences"][0]["canvases"]):
            self.assertEqual(
                canvas["label"],
                iiif_images[idx]["label"],
                "canvas label should match image label",
            )
    ...
    def test_manifest_attributes(self):
        ...
        manifest = self.create_manifest(
            request, manifest_id, label=label, description=description
        )
        md = manifest.to_dict()
