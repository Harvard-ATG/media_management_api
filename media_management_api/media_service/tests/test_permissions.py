...
class GenericCoursePermissionView(GenericAPIView):
    ...
    def get(self, request, pk=None):
        course = self.get_object() # expected to call check_object_permissions() implicitly
        return Response({"course_id": course.pk})

    def delete(self, request, pk=None):
        course = Course.objects.get(pk=pk)
        self.check_object_permissions(request, course) # explicitly calling check_object_permissions()
        course.delete()
...
class TestGenericUserHasObjectPermission(APITestCase):
    ...
    def setUp(self):
        ...
        CourseUser.add_to_course(is_admin=False, user_profile=self.member_user_profile, course_id=self.course.pk)
        CourseUser.add_to_course(is_admin=True, user_profile=self.admin_user_profile, course_id=self.course.pk)

        self.view = GenericCoursePermissionView.as_view()
        self.request_map = {
            'get': APIRequestFactory().get('/api-test/courses/%d' % self.course.pk),
            'delete': APIRequestFactory().delete('/api-test/courses/%d' % self.course.pk),
        }
