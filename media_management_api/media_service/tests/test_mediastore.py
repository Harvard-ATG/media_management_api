...
from ..mediastore import MediaStoreException, MediaStoreUpload
...
TEST_FILES = {
    "test.png": {
        ...
        "content": base64.b64decode(
            "iVBORw0KGgoAAAANSUhEUgAAABgAAAAYBAMAAAASWSDLAAAAFVBMVEUAAAD///8fHx9fX19/f38/Pz+fn5+WwvWOAAAAPElEQVQYlWNgGHjAZGIMocEkswKYYoFxjA2UTdVMIRxmBSMjuAyLsRGzEZwD0gLjMLkaKBuzmsKMp4WbAVr9BJo70GnDAAAAAElFTkSuQmCC"
        ),
...
class TestMediaStoreUpload(unittest.TestCase):
    ...
    def testCreateFileName(self):
        test_file = self.test_files["test.png"]
        instance = self.createMediaStoreUpload(test_file)
        expected_filename = "%s.%s" % (
            instance.getFileHash(),
    ...
    def testCreateMediaStoreInstance(self):
        ...
        self.assertEqual(
            media_store.file_extension, media_store_upload.getFileExtension()
        )
    ...
    def testImageJpegExtensionNormalized(self):
        ...
        self.assertEqual(
            media_store_upload.getFileExtension(), normalized_jpeg_extension
        )
        self.assertTrue(media_store_upload.validateImageExtension())
...
class TestUtilFunctions(unittest.TestCase):
    def testGuessImageExtensionFromUrl(self):
        ...
        self.assertEqual(
            "jpeg",
            guess(
                "https://example.com/profile_images/466953024271679488/3rftwYWT.jpeg"
            ),
        )
        self.assertEqual(
            "jpg",
            guess(
                "https://example.com/commons/thumb/c/c7/Harvard_square_harvard_yard.JPG/300px-Harvard_square_harvard_yard.JPG"
            ),
        )
        self.assertEqual(
            "gif",
            guess(
                "http://example.com/static/xyz/files/horizontal_large_nobg.gif?m=1489563850&_q=1"
            ),
        )
        self.assertEqual(
            "png",
            guess(
                "https://site.example.com/sites/default/files/_landing_carousel/Ext-538-carousel.png"
            ),
        )
        self.assertEqual(
            None,
            guess(
                "http://static1.example.com/static/5581f1f1e4b0be63c4780f1a/t/5584a36c/?format=1500w"
            ),
        )
...
class TestUrlImport(unittest.TestCase):
    ...
    def testProcessRemoteImages(self, mock_fetch):
        ...
        for item in request_data["items"]:
            ...
            self.assertEqual(
                sorted(processed[url]["data"].keys()), ["description", "title"]
            )
            self.assertEqual(processed[url]["data"]["title"], item["title"])
            if "description" in item:
                self.assertEqual(
                    processed[url]["data"]["description"], item["description"]
                )
            else:
