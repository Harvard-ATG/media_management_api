...
class IsCourseUserAuthenticated(BasePermission):
    def has_permission(self, request, view):
        ...
        logger.info(
            "user %s has_permission for request [%s %s] => %s"
            % (
    ...
    def has_object_permission(self, request, view, object):
        ...
        logger.info(
            "user %s has_object_permission for object [%s] request [%s %s] => %s"
            % (request.user.id, object, request.method, request.path, has_perm)
        )
        return has_perm
